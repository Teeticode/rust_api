...
use crate::{
    ...
    mail::{
        mails::{send_forgot_password_email, send_verification_email, send_welcome_email},
        sendotp::send_otp,
    },
    models::{SignInRequest, VerifyOtp},
...
pub async fn login(
    ...
    let result = app_state
        .db_client
        .get_user(None, None, Some(&body.email), None, None)
        .await
...
pub async fn verify_email(
    ...
    let result = app_state
        .db_client
        .get_user(None, None, None, Some(&query_params.token), None)
        .await
        .map_err(|e| HttpError::server_error(e.to_string()))?;
...
pub async fn reset_password(
    ...
    let result = app_state
        .db_client
        .get_user(None, None, None, Some(&body.token), None)
        .await
        .map_err(|e| HttpError::server_error(e.to_string()))?;
...
pub async fn send_otp_phone(
    service: Extension<TwillioService>,
    Json(data): Json<SignInRequest>,
) -> Result<Json<String>, StatusCode> {
    let number = verify(data);
    match service.send_otp(number).await {
        Ok(_) => Ok(Json("Opt has been sent to number".parse().unwrap())),
        Err(ex) => {
            eprintln!("{:?}", ex);
...
async fn verify_otp(
    ...
    match service.verify_otp(data).await {
        Ok(_) => Ok(Json(
            "Otp has been verified successfully!!".parse().unwrap(),
        )),
        Err(ex) => {
            eprintln!("{:?}", ex);
...
fn verify(sign_in_request: SignInRequest) -> String {
    //simulate verify
    //simulate get phone number against email
    String::from(sign_in_request.phone.to_string())
}
